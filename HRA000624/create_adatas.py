...
base_data_dir = '../../../data/immune_ageing/HRA000624/'
print(base_data_dir)
...
def create_adatas(data_dir, metadata, out_dir):
  for sample_idx in range(0, len(metadata)):
    ...
    sample_accession = sample_metadata['sample_accession']
    print("%s: %s" % (sample_name, sample_accession))

    sample_adata = sc.read_10x_mtx(
      os.path.join(data_dir, sample_accession, 'outs', 'filtered_feature_bc_matrix'),
      cache=True
    )
